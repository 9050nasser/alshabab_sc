...
frappe.ui.form.on('Requests', {
	...
	refresh(frm) {
		...
		check(frm);
		debugger;
		if (frappe.user_roles.includes("CEO") || frappe.user_roles.includes("رئيس كرة القدم النسائية")) {
			cur_frm.page.add_menu_item(__("توجيه"), function () {
				...
				var d = new frappe.ui.Dialog({
					'fields': [
						...
						{
							'fieldname': 'department', fieldtype: 'MultiSelectList', 'label': 'Department',
							get_data: function (txt) {
								return frappe.db.get_link_options('Department', txt);
								// return frappe.get_all("Departmant", fields = ["name"], pluck = "name")
